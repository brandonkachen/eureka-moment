...
You may have noticed some strange extra characters like `*` or `&` in the code above; we'll
explain what that all is in the next section. It's time to refactor!
...
Remember the `*` in the `arr` parameter in the `display` function? That's not a typo;
it's a C++ **pointer**. This is one of the coolest, most dangerous, and most unique
ideas of C and C++. Let's tread carefully.

At the most basic level, pointers are just memory addresses; they tell you about a location in
computer memory, kind of like street addresses tell you about a location in the real world.
Try printing out `gb` in your `display(...)` function. You should get something that looks
like this: `0x78f4eeaf3fa0`. That's a memory address! This isn't terribly exciting by itself,
and in day-to-day coding, you won't need to directly interact with these values yourself.

Things get more interesting when you look at the underlying piece of memory. Like street
addresses, memory addresses map to a location that contains some data. This is where pointers
get their name: they "point" to your data in memory. Think of them as tiny maps or
breadcrumbs for your program; when you have a pointer, you can "follow" it to get to the
underlying piece of memory and read those values. This process of folliwng a pointer is
called **dereferencing**.

That means in our code above, we weren't really copying the entire array into the `display`
function when we passed it as a parameter. We were actually copying the pointer value,
which is very small in comparison. When we access a specific index in the `arr` variable
inside `display`, we dereference the pointer and get to the array value underneath.

This has huge implications for performance. Pointers are one of the key reasons for C and C++'s
legendary speed because they're used _everywhere_. Now, we can reach objects of any size
just as quickly as we can reach an int!
...
should do, so it's up to your compiler. Compilers can do anything they want, like reading **garbage values**
(a fancy way of saying useless data) or crash the program if they wanted to. The key is that,
as the programmer, you don't know how your program will behave when compiled by an arbitrary compiler.
...
p = &i; // we just want the memory address, not the value of i itself
...
// 2.
int* q = p + 2; // code specified index 2
...
Armed with our new knowledge, we see that our code calls the `display(...)` function
and passes in a reference to the `gameState` array. The function immediately dereferences
it to get `gb` and `gs`. Our previous examples were a lot simpler, what gives? Well,
as we discussed at the beginning of this level, 2D arrays are arrays containing arrays.
That means //TODO

**Task**: Print out the pointer values for `gb` and `gs`. Add a comment that
steps through the entire process that your program goes through when the following
code runs inside the `display(...)` function:
